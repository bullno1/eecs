...
typedef struct eecs_table_s {
	...
	eecs_array(eecs_system_entity_callback_t) init_callbacks;
	eecs_array(eecs_system_entity_callback_t) cleanup_callbacks;
...
eecs_try_match_system_with_table(
	...
	if (system_options->init_per_entity_fn) {
		eecs_array_push(memctx, table->init_callbacks, ((eecs_system_entity_callback_t) {
	...
	if (system_options->cleanup_per_entity_fn) {
		eecs_array_push(memctx, table->cleanup_callbacks, ((eecs_system_entity_callback_t) {
...
eecs_destroy_entity_now(eecs_world_t* world, eecs_entity_data_t* entity_data) {
	...
	// Cleanup components
	for (eecs_id_t i = table->signature.length - 1; i >= 0; --i) {
		eecs_component_t component = table->signature.components[i];
		const eecs_component_options_t* component_options = &ecs->components[eecs_index_of(component)];

		if (component_options->cleanup_fn) {
			char* component_data = chunk
				+ table->component_storage_offsets[i]
				+ pos_in_chunk * component_options->size;

			component_options->cleanup_fn(world, handle, component_data, component_options->userdata);
		}
	}
	entity_data = &world->entities[from_1_index - 1];
	...
	for (eecs_id_t i = 0; i < table->signature.length; ++i) {
		eecs_component_t component = table->signature.components[i];
		size_t component_size = ecs->components[eecs_index_of(component)].size;
		char* component_data = chunk
			+ table->component_storage_offsets[i]
	...
	world->next_free_entity_slot = entity_data - world->entities + 1;
}
...
eecs_create_entity_for_table(
	...
	entity_data->pos_in_table = pos_in_table;
	...
	if (chunk_index >= eecs_array_length(table->chunks)) {
		...
		eecs_array_push(memctx, table->chunks, chunk);
	} else {
	...
	entity_ids[pos_in_chunk] = entity_handle.from_1_index;
	...
	for (eecs_id_t i = 0; i < table->signature.length; ++i) {
		...
		if (init_data == NULL) {
			memset(component_data, 0, component_options->size);
		} else {
			memcpy(component_data, init_data, component_options->size);
		}
	}
	...
	for (eecs_id_t i = 0; i < table->signature.length; ++i) {
		...
		if (component_options->init_fn) {
			...
			component_options->init_fn(
				world, entity_handle, component_data, component_options->userdata
			);
		}
	}

	// Init entity by systems
	eecs_array_indexed_foreach(eecs_system_entity_callback_t, itr, table->init_callbacks) {
...
eecs_destroy_world(eecs_world_t* world) {
	...
	eecs_array_indexed_foreach(eecs_table_t*, itr, world->tables) {
		...
		eecs_array_free(memctx, table->init_callbacks);
		eecs_array_free(memctx, table->cleanup_callbacks);
...
eecs_create_entity_from_template(
	...
	if (overrides != NULL) {
		...
		memcpy(init_data, template_data->init_data, sizeof(eecs_component_init_t) * table->signature.length);
